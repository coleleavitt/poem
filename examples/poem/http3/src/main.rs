...
//!
//! This example demonstrates how to create an HTTP/3 server using poem.
//!
//! To test this example:
//! 1. Run the server: `cargo run -p example-http3`
//! 2. Use curl with HTTP/3 support: `curl --http3 https://localhost:3000/`
//!
//! Note: You'll need a curl version with HTTP/3 support (curl 7.66.0+ with nghttp3).
//! Alternatively, you can use a browser that supports HTTP/3 (Chrome, Firefox, Edge).

use poem::{
    get, handler,
    h3::Http3Server,
    listener::quinn::{QuinnConfig, QuinnListener},
    Route,
};

// Self-signed certificate for testing
// DO NOT use in production - generate proper certificates!
const CERT: &str = r#"
-----BEGIN CERTIFICATE-----
...
pd3MRlu+4QFrveExwxgdahXvnEdVBGkpoWoR6kHDAaMRgwFgYDVQQDDA9wb255
dG93biBSU0EgQ0GCAXswOwYDVR0RBDQwMoIOdGVzdHNlcnZlci5jb22CFXNlY29u
ZC50ZXN0c2VydmVyLmNvbYIJbG9jYWxob3N0MA0GCSqGSIb3DQEBCwUAA4IBgQBs
K1gTvwh8x4LMe24lROW/LGWDRAyhaI8qDxxlitm0DPxU8p4iQoDQi3Z+oVKqsSwJ
...
#[handler]
fn hello(name: poem::web::Path<String>) -> String {
    format!("Hello, {}! (via HTTP/3)", name.0)
}
...
async fn main() -> Result<(), std::io::Error> {
    ...
    // Configure QUIC/HTTP3 with TLS
    let config = QuinnConfig::new().cert(CERT).key(KEY);

    // Create the listener and server
    let listener = QuinnListener::bind("0.0.0.0:3000", config)?;

    println!("HTTP/3 server listening on https://localhost:3000");
    println!("Test with: curl --http3 -k https://localhost:3000/");

    Http3Server::new(listener).name("http3-example").run(app).await
}
